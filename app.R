...
cant_Heroes = length(data_Keys_Heroes$primaryname)
...
LstNameMinErrorGB = readRDS("lstNameMinErroGB.RDS")
...
encontrarGanador = function(tablero){
  if(is_done(tablero)){
    ...
    for (i in 1:(length(data_partidas)-1)){
      ...
      if(i == 1){
        #Se establecen los nombres de los niveles para que concuerden con todas las instancias de consulta
    ...
    puntuacion_victoria = round(predict(LstGrndModel[[which(niveles_maps == tablero[[5]])]], slq_draft[,-c(1,2)], n.trees = as.numeric(LstNameMinErrorGB[[which(niveles_maps == tablero[[5]])]]))-1)
...
rollout = function(tablero, booleano_politica = TRUE){
  ...
  resultado = encontrarGanador(tablero)
  if(resultado == 1){
    return(1)}
  else{
    return(-1)}
